...
def play():
    global file, length, conversionfactor, previous, metadata
    if locationbox.get() != "":
    ...
    if file != previous:
        if platform == "Windows":
            ...
            length = MP3("song.ogg").info.length
...
volumeslider = tk.Scale(root, from_ = 0, to = 1, resolution = 0.01, bg="grey", fg="white", bd=0, troughcolor="black",
                        showvalue=0, width=10,highlightcolor="grey", highlightbackground="grey", sliderlength=10, command=change_volume)
volumeslider.set(1)
...
durationslider = tk.Scale(root, from_ = 0, to = 1, resolution = 0.001, bg="grey", fg="white", bd=0, troughcolor="black",
                        showvalue=0, width=10, sliderlength=7, highlightcolor="grey", highlightbackground="grey", orient= tk.HORIZONTAL, length=200, command=change_location)
...
# clock = tk.Label(frame, font = ('consolas', 200, 'bold'), #change font size to 200 for final program
#                  bg = 'grey', fg = 'white')
